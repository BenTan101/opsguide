...
router.get("/get-module-data", async function (req, res, next) {
  try {
    let details = await opsguide.getModuleData();
    res.json(details);
    return details;
  } catch (err) {
    console.error(`Error getting module data`, err.message);
    next(err);
  }
});

router.get("/get-opportunity-data", async function (req, res, next) {
  try {
    let details = await opsguide.getOpportunityData();
    res.json(details);
    return details;
  } catch (err) {
    console.error(`Error getting opportunity data`, err.message);
    next(err);
  }
});
